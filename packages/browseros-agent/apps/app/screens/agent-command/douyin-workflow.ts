...
- 允许在内存中维护一个轻量 results 数组，但只保留最终导出 Markdown 所需字段，禁止缓存完整 DOM、大段页面文本、截图或原始工具返回
...
4. 初始化一个轻量 results 数组，用来保存最终 Markdown 表格行；数组中每条记录只能包含：序号、视频名称、视频链接、点赞数、评论总数、收藏数、转发数、失败原因。
...
7. 当前视频读取完成后，只把这一条的最终字段写入 results 数组。某个字段读取不到时填 null，但仍保留该视频；写入后立即丢弃当前详情页的临时 DOM、工具原始返回和中间推理，不要把它们保存在结果里。
8. 当前视频记录加入 results 后，立即调用 close_page(page=详情页ID) 关闭当前详情标签页，再调用 show_page(page=listPage) 回到原搜索结果页并确认详情页已关闭，然后才允许处理下一个候选视频。禁止在详情页直接跳转到下一条，禁止同时打开多个详情页，任何时刻最多只能存在 listPage 和一个详情页。
...
- 每条详情固定执行：new_page -> 读取单条 -> 仅保存最终字段到 results -> 清除单条临时数据 -> close_page -> show_page(listPage)，确认返回列表后再继续
- 数量字段同时保留页面原始文本和可解析的整数值；无法确认时填 null，不要猜测
- 候选列表建立后先报告“已找到 N/targetCount 条”；采集过程中定期报告简短进度
- 必须保持“最多点赞”排序后的页面顺序，不要自行重新排序

结果文件：
- 全部视频处理完成后，再生成 Markdown 文件 douyin-top<targetCount>-<关键词>-<日期-时分>.md；文件名中的时间至少精确到小时和分钟，避免同一天重复执行时覆盖
...
- 写文件前先根据 results 重新组装一份完整 Markdown 文本，再一次性写入文件，禁止边采集边追加 Markdown
